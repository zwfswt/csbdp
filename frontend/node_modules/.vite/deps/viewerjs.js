...
//#region node_modules/viewerjs/dist/viewer.js
var require_viewer = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/*!
	* Viewer.js v1.11.7
		...
		function forEach(data, callback) {
			if (data && isFunction(callback)) {
				if (Array.isArray(data) || isNumber(data.length)) {
					...
					var i;
					for (i = 0; i < length; i += 1) if (callback.call(data, data[i], i, data) === false) break;
		...
		*/
		...
		var render = {
			...
			renderList: function renderList() {
				...
				setStyle(this.list, assign({ width: outerWidth * this.length - gutter }, getTransforms({ translateX: (this.viewerData.width - offsetWidth) / 2 - outerWidth * index })));
			},
			...
			renderImage: function renderImage(done) {
				...
				if (done) if ((this.viewing || this.moving || this.rotating || this.scaling || this.zooming) && this.options.transition && hasClass(image, CLASS_TRANSITION)) {
		...
		var handlers = {
			click: function click(event) {
				...
				switch (action) {
					case "mix":
						...
						else if (options.inline) if (this.fulled) this.exit();
						else this.full();
						else this.hide();
						break;
					case "hide":
			...
			loadImage: function loadImage(event) {
				...
				getImageNaturalSizes(image, this.options, function(naturalWidth, naturalHeight) {
					...
					if (parentHeight * aspectRatio > parentWidth) if (filled) width = parentHeight * aspectRatio;
			...
			pointerup: function pointerup(event) {
				...
				if (IS_TOUCH_DEVICE && action !== ACTION_ZOOM && pointer && Date.now() - pointer.timeStamp < 500) {
					...
					if (options.toggleOnDblclick && this.viewed && event.target === this.image) if (this.imageClicked) {
			...
			wheel: function wheel(event) {
				...
				var ratio = Number(this.options.zoomRatio) || .1;
				var delta = 1;
		...
		var methods = {
			...
			*/
			view: function view() {
				...
				this.viewing = { abort: function abort() {
			...
			*/
			zoomTo: function zoomTo(ratio) {
				...
				if (isNumber(ratio) && this.viewed && !this.played && (_zoomable || options.zoomable)) {
					if (!_zoomable) {
						var minZoomRatio = Math.max(.01, options.minZoomRatio);
						var maxZoomRatio = Math.min(100, options.maxZoomRatio);
						ratio = Math.min(Math.max(ratio, minZoomRatio), maxZoomRatio);
					}
					if (_originalEvent) switch (_originalEvent.type) {
						case "wheel":
						...
						case "touchmove":
						case "mousemove":
							if (ratio > .99 && ratio < 1.01) ratio = 1;
							break;
					}
			...
			*/
			...
			tooltip: function tooltip() {
				...
				if (!this.tooltipping) if (options.transition) {
			...
			*/
			toggle: function toggle() {
				...
				if (this.imageData.ratio === 1) this.zoomTo(this.imageData.oldRatio, true, null, _originalEvent);
				else this.zoomTo(1, true, null, _originalEvent);
				return this;
			},
			...
			update: function update() {
				...
				if (this.ready) {
					...
					forEach(this.items, function(item, i) {
						...
						var image = images[i];
						if (image && img) {
							if (image.src !== img.src || image.alt !== img.alt) changedIndexes.push(i);
						} else changedIndexes.push(i);
					});
					setStyle(this.list, { width: "auto" });
					this.initList();
					if (this.isShown) if (this.length) {
						if (this.viewed) {
		...
		var others = {
			...
			requestFullscreen: function requestFullscreen(options) {
				...
				if (this.fulled && !(document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement)) {
					...
					if (documentElement.requestFullscreen) if (isPlainObject(options)) documentElement.requestFullscreen(options);
