...
var _TreeNodeValueBase = class {
	...
	*/
	get fullPath() {
		...
		return require_utils.joinPath(this.parent?.fullPath ?? "", pathSegment);
	}
	/**
	...
	*/
	...
	get overrides() {
		return [...this._overrides.entries()].sort(([nameA], [nameB]) => nameA === nameB ? 0 : nameA === CONVENTION_OVERRIDE_NAME || nameA !== EDITS_OVERRIDE_NAME && (nameA < nameB || nameB === EDITS_OVERRIDE_NAME) ? -1 : 1).reduce((acc, [_path, routeBlock]) => {
			return require_utils.mergeRouteRecordOverride(acc, routeBlock);
		}, {});
	}
	...
	*/
	mergeOverride(filePath, routeBlock) {
		...
		this._overrides.set(filePath, require_utils.mergeRouteRecordOverride(existing, routeBlock));
	}
	/**
...
function createTreeNodeValue(segment, parent, opts = {}) {
	...
	if (options.format === "file" && openingPar >= 0) {
		...
		if (closingPar < 0 || closingPar < openingPar) {
			require_utils.warn(`Segment "${segment}" is missing the closing ")". It will be treated as a static segment.`);
	...
	function consumeBuffer() {
		if (state === ParseRawPathSegmentState.static) subSegments.push(buffer);
			...
			if (!currentTreeRouteParam.paramName) {
				require_utils.warn(`Invalid parameter in path "${segment}": parameter name cannot be empty. Using default name "pathMatch" for ':()'.`);
...
var TreeNode = class TreeNode {
	...
	*/
	get regexp() {
		...
		return "/^" + (re.startsWith("(?:\\/") ? "" : "\\/") + re.replace(require_utils.ESCAPED_TRAILING_SLASH_RE, "") + "$/i";
	}
	/**
...
function generateCustomParamParsersList(paramParsers) {
	...
	return parserNames.map(require_utils.toStringLiteral);
}
...
${node.getChildrenSorted().map((n) => generateRouteNamedMap(n, options, paramParsersMap)).join("")}}`;
	return (node.value.components.size && node.isNamed() ? require_utils.pad(2, `${require_utils.toStringLiteral(node.name)}: ${generateRouteRecordInfo(node, options, paramParsersMap)},\n`) : "") + (node.children.size > 0 ? node.getChildrenSorted().map((n) => generateRouteNamedMap(n, options, paramParsersMap)).join("\n") : "");
}
function generateRouteRecordInfo(node, options, paramParsersMap) {
	...
	const typeParams = [
		require_utils.toStringLiteral(node.name),
		require_utils.toStringLiteral(node.fullPath),
	...
	typeParams.push(require_utils.formatMultilineUnion(childRouteNames.map(require_utils.toStringLiteral), 4));
	return `RouteRecordInfo<
${typeParams.map((line) => require_utils.pad(4, line)).join(",\n")}
  >`;
}
...
${Array.from(routesInfo.entries()).map(([file, { routes, views }]) => `
  ${require_utils.toStringLiteral(file)}: {
    routes:
      ${require_utils.formatMultilineUnion(routes.sort().map(require_utils.toStringLiteral), 6)}
    views:
      ${require_utils.formatMultilineUnion(views.sort().map(require_utils.toStringLiteral), 6)}
  }`).join("\n")}
}`;
}
/**
...
${node.getChildrenSorted().map((child) => generateRouteRecords(child, options, importsMap, indent + 1)).join(",\n")}
	...
	const startIndent = require_utils.pad(indent * 2);
	const indentStr = require_utils.pad((indent + 1) * 2);
	const overrides = node.value.overrides;
	const routeRecord = `${startIndent}{
${indentStr}path: ${require_utils.toStringLiteral(node.path)},
${indentStr}${node.value.components.size ? node.isNamed() ? `name: ${require_utils.toStringLiteral(node.name)},` : `/* no name */` : `/* internal name: ${typeof node.name === "string" ? require_utils.toStringLiteral(node.name) : node.name} */`}
...
${files.map(([key, path]) => `${indentStr + "  "}${require_utils.toStringLiteral(key)}: ${generatePageImport(path, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
...
function generatePageImport(filepath, importMode, importsMap) {
	if ((typeof importMode === "function" ? importMode(filepath) : importMode) === "async") return `() => import(${require_utils.toStringLiteral(filepath)})`;
...
function parseCustomBlock(block, filePath, options) {
	...
	if (lang === "json5") try {
		...
		require_utils.warn(`Invalid JSON5 format of <${block.type}> content in ${filePath}\n${err.message}`);
	}
	else if (lang === "json") try {
		return JSON.parse(block.content);
	} catch (err) {
		require_utils.warn(`Invalid JSON format of <${block.type}> content in ${filePath}\n${err.message}`);
	}
	else if (lang === "yaml" || lang === "yml") try {
		return (0, yaml.parse)(block.content);
	} catch (err) {
		require_utils.warn(`Invalid YAML format of <${block.type}> content in ${filePath}\n${err.message}`);
	}
	else require_utils.warn(`Language "${lang}" for <${block.type}> is not supported. Supported languages are: json5, json, yaml, yml. Found in in ${filePath}.`);
}
...
var RoutesFolderWatcher = class {
	...
	on(event, handler) {
		this.watcher.on(event, (filePath) => {
			...
			handler({
				filePath,
				routePath: require_utils.asRoutePath({
					src: this.src,
					path: this.path,
...
function resolveFolderOptions(globalOptions, folderOptions) {
	...
	return {
		...
		pattern: require_utils.appendExtensionListToPattern(filePatterns, extensions),
...
function definePageTransform({ code, id }) {
	...
	try {
		...
		require_utils.warn(`[${id}]: Failed to process definePage: ${error instanceof Error ? error.message : "Unknown error"}`);
	...
	if (isExtractingDefinePage) {
		...
		try {
			...
			require_utils.warn(`[${id}]: ${error instanceof Error ? error.message : "Invalid scope reference in definePage"}`);
...
function extractDefinePageInfo(sfcCode, id) {
	...
	try {
		...
		require_utils.warn(`[${id}]: Failed to extract definePage info: ${error instanceof Error ? error.message : "Unknown error"}`);
		return;
	}
	if (!ast) return;
	...
	for (const prop of routeRecord.properties) if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") if (prop.key.name === "name") if (prop.value.type !== "StringLiteral" && (prop.value.type !== "BooleanLiteral" || prop.value.value !== false)) require_utils.warn(`route name must be a string literal or false. Found in "${id}".`);
	else routeInfo.name = prop.value.value;
	else if (prop.key.name === "path") if (prop.value.type !== "StringLiteral") require_utils.warn(`route path must be a string literal. Found in "${id}".`);
	...
	for (const prop of queryObj.properties) if (prop.type === "ObjectProperty" && prop.key.type === "Identifier") {
		...
		if (prop.value.type === "StringLiteral") queryParams[paramName] = { parser: prop.value.value };
			...
			for (const paramProp of prop.value.properties) if (paramProp.type === "ObjectProperty" && paramProp.key.type === "Identifier") {
				...
				else require_utils.warn(`Unrecognized default value in definePage() for query param "${paramName}". Typeof value: "${paramProp.value.type}". This is a bug or a missing type of value, open an issue on https://github.com/vuejs/router and provide the definePage() code.`);
			}
...
function extractRouteAlias(aliasValue, id) {
	if (aliasValue.type !== "StringLiteral" && aliasValue.type !== "ArrayExpression") require_utils.warn(`route alias must be a string literal or an array of string literals. Found in "${id}".`);
	else return aliasValue.type === "StringLiteral" ? [aliasValue.value] : aliasValue.elements.filter((node) => {
		if (node?.type === "StringLiteral") return true;
		require_utils.warn(`route alias array must only contain string literals. Found ${node?.type ? `"${node.type}" ` : ""}in "${id}".`);
		return false;
	}).map((el) => el.value);
}
...
function generateRouteRecord({ node, parentVar, parentNode, state, options, importsMap, paramParsersMap }) {
	...
	if (!shouldSkipNode) {
		...
		if (isMatchable) {
			...
			recordName = `name: ${require_utils.toStringLiteral(node.name)},`;
		} else recordName = node.name ? `/* (internal) name: ${require_utils.toStringLiteral(node.name)} */` : `/* (removed) name: false */`;
...
function generateRouteRecordComponent(node, indentStr, importMode, importsMap) {
	...
	return `components: {
${Array.from(node.value.components).map(([key, path]) => `${indentStr + "  "}${require_utils.toStringLiteral(key)}: ${generatePageImport(path, importMode, importsMap)}`).join(",\n")}
${indentStr}},`;
}
/**
...
function generatePathCode(node, importsMap, paramParsersMap) {
	...
	else return `path: new MatcherPatternPathStatic(${require_utils.toStringLiteral(node.fullPath)}),`;
}
/**
...
function createRoutesContext(options) {
	...
	async function scanPages(startWatchers = true) {
		...
		await Promise.all([...routesFolder.map((folder) => resolveFolderOptions(options, folder)).map((folder) => {
			...
			return (0, tinyglobby.glob)(folder.pattern, {
				...
				routePath: require_utils.asRoutePath(folder, file),
				filePath: file
			}))));
			...
			return scanParamParserFiles(folder, paramParsersInclude, paramParsersExclude).then((paramParserFiles) => {
	...
	function generateResolver() {
		const importsMap = new require_utils.ImportsMap();
...
`;
	...
	async function _writeConfigFiles() {
		...
		require_utils.logTree(routeTree, logger.log);
		if (dts) {
	...
	const writeConfigFiles = require_utils.throttle(_writeConfigFiles, 500, 100);
...
Object.defineProperty(exports, 'DEFINE_PAGE_QUERY_RE', {
  enumerable: true,
  get: function () {
    return DEFINE_PAGE_QUERY_RE;
  }
});
...
Object.defineProperty(exports, 'MODULE_RESOLVER_PATH', {
  enumerable: true,
  get: function () {
    return MODULE_RESOLVER_PATH;
  }
});
Object.defineProperty(exports, 'MODULE_ROUTES_PATH', {
  enumerable: true,
  get: function () {
    return MODULE_ROUTES_PATH;
  }
});
Object.defineProperty(exports, 'ROUTES_LAST_LOAD_TIME', {
  enumerable: true,
  get: function () {
    return ROUTES_LAST_LOAD_TIME;
  }
});
Object.defineProperty(exports, 'ROUTE_BLOCK_ID', {
  enumerable: true,
  get: function () {
    return ROUTE_BLOCK_ID;
  }
});
Object.defineProperty(exports, 'VIRTUAL_PREFIX', {
  enumerable: true,
  get: function () {
...
Object.defineProperty(exports, 'asVirtualId', {
  enumerable: true,
  get: function () {
    return asVirtualId;
  }
});
Object.defineProperty(exports, 'createAutoExportPlugin', {
  enumerable: true,
  get: function () {
    return createAutoExportPlugin;
  }
});
...
Object.defineProperty(exports, 'createViteContext', {
  enumerable: true,
  get: function () {
    return createViteContext;
  }
});
Object.defineProperty(exports, 'getVirtualId', {
  enumerable: true,
  get: function () {
    return getVirtualId;
  }
});
Object.defineProperty(exports, 'routeBlockQueryRE', {
  enumerable: true,
  get: function () {
    return routeBlockQueryRE;
  }
});